...
def _extract_connection_line(text: str) -> str:
    """
...
def read_xlsx(filename: str) -> pandas.DataFrame:
    x = pandas.DataFrame([])
    try:
        x = pandas.read_excel(filename, engine='openpyxl')
...
def write_xlsx(filename: str, x: pandas.DataFrame) -> None:
    try:
        x.to_excel(filename, index=False, engine='openpyxl')
    except OSError as err:
